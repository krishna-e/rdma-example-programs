...
{ 
   ...
   mr = ibv_reg_mr(pd, buf, 2 * sizeof (uint32_t), 
    ...
    if (!mr) 
        return 1;
   printf("local key:%llx\n", (unsigned long long)mr->lkey); 
    ...
    rep_pdata.buf_va = be64toh((unsigned long long) buf); 
    rep_pdata.buf_rkey = be32toh(mr->rkey); 
    ...
    printf("[%s]:Received integer %d from client!\n",(wc.opcode==IBV_WC_RECV)?"IB_WC_RECV":"UNEXPECTED_WC",ntohl(buf[0]));
    printf("Received integer %d from client!, invalidated_rkey:%llx\n",ntohl(buf[0]), (wc.wc_flags & IBV_WC_WITH_INV)?(unsigned long long)wc.invalidated_rkey:0xabab);
